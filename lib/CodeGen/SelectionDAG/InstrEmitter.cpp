  ...
  if (II.isVariadic())
    ...
    if (!(NumMIOperands >= II.getNumOperands() &&
           NumMIOperands <= II.getNumOperands() + II.getNumImplicitDefs() +
                            NumImpUses))
    {
      errs() << "NumMIOperands = " << NumMIOperands << " getNumOperands = " << II.getNumOperands() << " getNumImplicitDefs = "
        << II.getNumImplicitDefs() << " NumImpUses = " << NumImpUses << " NumDefs " << NumDefs << "\n";
      Node->dump();
      assert(0 && "#operands for dag node doesn't match .td file!");
    }
